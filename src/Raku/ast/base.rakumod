...
class RakuAST::Node {
    ...
    # Recursively walks the tree finding nodes of the specified type that are
    # beneath this one. A node that matches the stopper function will *not* be returned
    # even if it satisfies the specified type and condition. The node's children shall not be
    # visited. The search is strict - that is to say, it starts at the children
    # of the current node, but doesn't consider the current one.
    # Note: this is more expensive than find-nodes due to calling $stopper on
    #       each node, instead of doing a simple type check. Also, the exclusivity
    #       doesn't make sense without a $stopper, and so it is required.
