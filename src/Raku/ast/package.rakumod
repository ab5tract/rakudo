  ...
  is RakuAST::Doc::DeclaratorTarget
{
    ...
    method ensure-installed(RakuAST::Resolver $resolver, RakuAST::IMPL::QASTContext $context) {
        unless $!installed {
            ...
            if $name && !$name.is-empty && !$name.is-anonymous {
                ...
                my $full-name := self.IMPL-FULL-NAME($resolver);
                nqp::say("installed full name: " ~ $full-name.canonicalize);
    ...
    method PERFORM-CHECK(RakuAST::Resolver $resolver, RakuAST::IMPL::QASTContext $context) {
        ...
        && !nqp::istype(self, RakuAST::Role) # No idea why roles are exempt
    ...
    method PRODUCE-META-OBJECT() {
        ...
        unless $type.HOW.is_composed($type) {
  ...
  is RakuAST::Package::Attachable
{
    ...
    method IMPL-FULL-NAME(RakuAST::Resolver $resolver) {

        my $full-name  := nqp::findmethod(RakuAST::Package, 'IMPL-FULL-NAME')(self, $resolver);
        my @name-parts := self.IMPL-UNWRAP-LIST($full-name.parts);

        my $parameterizations := RakuAST::Name::Part::Parameterization.new;
        for self.IMPL-UNWRAP-LIST(self.parameterization.parameters) -> $p {
#            nqp::say("type/base-type: " ~ nqp::can($t, 'type') ~ "/" ~ nqp::can($t.type, 'base-type'));
#            next unless nqp::can($t, 'type') && nqp::can($t.type, 'base-type');

            #            my $base-type-name := self.IMPL-UNWRAP-LIST($t.type.base-type.name.parts)[0].name;
            #            nqp::say("base-type: " ~ $base-type-name ~ " first-part: " ~ @name-parts[0].name);
            #            next unless $base-type-name eq @name-parts[0].name;

            for self.IMPL-UNWRAP-LIST($p.type-captures) -> $tc {
                nqp::say("adding to name: " ~ $tc.name.canonicalize);
                $parameterizations.add-name: "::" ~ $tc.name.canonicalize
            }
        }

        RakuAST::Name.new: |self.IMPL-UNWRAP-LIST($full-name.parts), $parameterizations
    }
...
class RakuAST::Class
  is RakuAST::Package::Attachable
{
    has RakuAST::Package $!group-package;
    ...
    method IMPL-FULL-NAME(RakuAST::Resolver $resolver) {
        ...
        my $parameterizations := RakuAST::Name::Part::Parameterization.new;
        for self.IMPL-UNWRAP-LIST(self.traits) -> $t {
            nqp::say("type/base-type: " ~ nqp::can($t, 'type') ~ "/" ~ nqp::can($t.type, 'base-type'));
            next unless nqp::can($t, 'type') && nqp::can($t.type, 'base-type');
...
#            next unless $base-type-name eq @name-parts[0].name;
            ...
            for self.IMPL-UNWRAP-LIST($t.type.args.args) {
                nqp::say("adding to name: " ~ $_.name.canonicalize);
                my $prefix := nqp::istype($_.compile-time-value.HOW, Perl6::Metamodel::GenericHOW)
                                ?? "::" !! "";
                $parameterizations.add-name: $prefix ~ $_.name.canonicalize
            }
        }
    ...
    method PRODUCE-META-OBJECT() {
        ...
        unless $type.HOW.is_composed($type) {
            ...
            $type.HOW.compose($type, :compiler_services(self.compiler-services));
            CATCH {
                nqp::bindattr(self, RakuAST::Package, '$!compose-exception', $_)
            }
        }
