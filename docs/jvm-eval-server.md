...
script takes its own ceiling (heap plus 2g off-heap: 512m thread stacks,
metaspace, Truffle code cache -- the server shot that day held 8.7g against
an 8g heap), adds the room every live `EvalServer` JVM can still grow into
(its ceiling minus its current RSS), and refuses with exit 75 when the sum
exceeds `MemAvailable`. When it does fit, the JVM runs under
`systemd-run --user --scope -p MemoryMax=<ceiling>`, so a runaway is killed
inside its own cgroup and the terminal survives. `systemd-run --scope`
execs in place, so the pid a caller gets is java's own; pipes and kills are
unaffected. The guard is generated by `tools/build/create-jvm-runner.pl`.
A refusal is the answer: stop a server or lower the heap; do not bypass it.
