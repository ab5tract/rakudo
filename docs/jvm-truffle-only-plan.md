...
| **5 reflection-free unit** | milestone 1 DONE 2026-09-09: nqp stage2 as artifacts (nqp `57460ccd7`), all 21 stage2/share-lib jars `unit.meta`-only (zero `.class`), t/nqp 115/115 through `nqp-j-gradle` (113/115 from the rakudo root: 019-file-ops and 063-slurp are cwd-relative); clean build 273 s, suite 487 s at 3 jobs; post-completion gate: see item 6 |
| **6 unit artifact, no class file** | milestone 1 DONE 2026-09-09: nqp stage2 as artifacts (nqp `57460ccd7`), all 21 stage2/share-lib jars `unit.meta`-only (zero `.class`), t/nqp 115/115 through `nqp-j-gradle` (113/115 from the rakudo root: 019-file-ops and 063-slurp are cwd-relative); clean build 273 s, suite 487 s at 3 jobs; milestones 2-4 (in-memory compiles, Rakudo units, stage0 + deletion) open. **Post-completion gate (2026-09-09), two blockers:** the Rakudo `make` needs one line: the Makefile's `J_NQP_RR` (`tools/templates/jvm/Makefile.in:31`) starts a JVM directly on main class `nqp`, which an artifact `nqp.jar` no longer has (`Could not find or load main class nqp`); with the artifact entry (`org.raku.nqp.runtime.unit.UnitMain <nqp.jar>`, verified by hand) the whole build goes through -- BOOTSTRAP v6c 339 s wall, CORE.c 509 s wall (Stage parse 386.954, optimize 37.123, qast 31.327, jast 33.336, classfile 2.942), CORE.d 22 s (parse 8.611), CORE.e (parse 39.608), `make` 981 s incremental / ~1163 s from the top. `t/01-sanity` is then **21/25** at 2 jobs in 147 s: the four `use`-ing files (53-transpose, 55-use-trace, 56-use-isms, 99-test-basic) all die in one CORE.c program, `nqpp: unknown tag 11142 at 79 of 1060 words`; one-liner reproducer `./rakudo-j -e 'say("abc".subst(/b/,"x"))'`. That program's header is `PARAMS 0 -1 0` and its body carries P6BINDSIG/P6TRYBINDSIG, i.e. it is a **custom_args block** (nqp `d13dcbf69`) -- not an artifact/class-road crossing; the strict-campaign handoff's "exact next step" (Rakudo `make` + `t/01-sanity` on the custom_args nqp) had never been run, and this gate is the first time it was. |
