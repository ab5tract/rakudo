...
| cold start (outside the nine) | **milestone 7 CLOSED 2026-09-16** (rakudo `6217a89e61` / nqp `a837bf1bb`). Phase 1 of lazy loading landed as Phase B (artifact v2, the mapped store, lazy bodies, site identity) and is clock-neutral by design; Phase C filled the dispatch table from a training run in both builds, and that is where the clock moved: cold `rakudo -e` **2.68 -> 2.247 s**, `recorded=` 4723 -> 195 on the trained program, hits 100711 -> 35512. **Phase 2 (SC demand deserialization), then what remained of the lazy-loading spec, closed with milestone 8 Phase C on 2026-09-19** (format 12 and the demand reader; see the Milestone 8 paragraph above). Left open at the close: the compression decision, training's ~1 % run-to-run variation (artifacts are no longer reproducible byte for byte), one new red (`closure-static-clone.t`), and the suite clock, not gathered because the sweep's single server wedged at file 310 of 482 |
