...
Correction (2026-09-16, after the close, from `journalctl`): the whole-t/ server was not "wedged"; the kernel's cgroup OOM killer killed it at 04:39:50 inside the `systemd-run --scope -p MemoryMax=9437184kB` cage `rakudo-eval-server` creates (heap 6 GB + off-heap allowance), anon-rss 8.99 GB, `TruffleCompiler` allocating, scope `Failed with result 'oom-kill'` after 55 min 15 s. `dmesg` is unreadable to the user (hence the subagent's "empty dmesg"); KDE's "Memory Shortage Avoided" notification at 04:39 was the tell. Ruling 15's guard and this cap are the same runner cage. Next suite attempt: raise the off-heap allowance (or the cap) for a long single-server run, or measure the native growth. The findings, the suite doc, the plan position, the spec and the memory were corrected in the same commit.
