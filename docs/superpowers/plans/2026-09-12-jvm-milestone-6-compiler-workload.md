...
Keep the knob if the wall clock drops. Record in the ledger: wall clock,
stage times, `failed` count (expect the "too large" group to vanish),
`total-compiler-ms`, and the verdict KEEP or DROP with the number that
justifies it.
